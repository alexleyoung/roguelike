...
int move(dungeon *d, character *c) {
    ...
    if (C_IS(c, TELEPATHIC) || los) {
        if (C_IS(c, TUNNELING)) {
            COPY_2D_ARRAY(c->dist_to_player, d->tunnel_dists, DUNGEON_HEIGHT, DUNGEON_WIDTH);
        } else {
...
int move_to(dungeon *d, character *c, point p) {
    ...
    if (d->character_map[p.r][p.c]) {
        int end = 0;
        if (d->character_map[p.r][p.c]->traits == PLAYER_TRAIT) end = 1; // end game

        // kill character
        printf("%d (sprite: %c) kills %d (sprite: %c) from (%d,%d) to (%d,%d)\n", c->id, c->sprite, d->character_map[p.r][p.c]->id,d->character_map[p.r][p.c]->sprite, c->pos.r, c->pos.c, p.r, p.c);
