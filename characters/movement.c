...
int move(dungeon *d, character *c) {
    ...
    // random move if no LOS and not telepathic
    ...
    // update character dist_to_player if telepathic
    if (C_IS(c, TELEPATHIC)) {
        if (C_IS(c, TUNNELING)) {
...
int move_random(dungeon *d, character *c) {
    ...
    } while ((new_r < 1 || new_r > DUNGEON_HEIGHT - 2 || new_c < 1 || new_c > DUNGEON_WIDTH - 2) || 
...
int move_to(dungeon *d, character *c, point p) {
    ...
    // if hardness still greater than 0, early exit
