...
void print_heap(j_heap_t *h, char *(*print)(const void *v))
{
  heap_node_t *n;
...
void heap_node_delete(j_heap_t *h, heap_node_t *hn)
{
  heap_node_t *next;
...
static void heap_link(j_heap_t *h, heap_node_t *node, heap_node_t *root)
{
...
static void heap_consolidate(j_heap_t *h)
{
  uint32_t i;
  heap_node_t *x, *y, *n;
...
static void heap_cut(j_heap_t *h, heap_node_t *n, heap_node_t *p)
{
  if (!--p->degree) {
    p->child = NULL;
  }
...
static void heap_cascading_cut(j_heap_t *h, heap_node_t *n)
{
  heap_node_t *p;
